...
const mailTo = (href) => {
  window.open(
    'mailto:conquerfitness01@gmail.com?subject=V3%20Online%20Fitness%20Program%20-%20Customer%20Contact',
    '_blank'
  )
}
  ...
  return (
    <SectionWrapper style={{ justifyContent: 'center' }}>
      <SectionMax>
        <Container
          p={[2, 4]}
          fluid={props.banner.bg__img.childImageSharp.fluid}
          borderRadius={['20px']}
        >
          <Overlay />
          <ContentContainer xs={10} md={5}>
            <Heading2 mb={4} textAlign={'left'} color={'white'}>
              {props.banner.title}
            </Heading2>
            {props.children}
          </ContentContainer>
